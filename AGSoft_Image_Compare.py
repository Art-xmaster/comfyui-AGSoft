...
class AGSoftImageCompare(PreviewImage):
    ...
    def INPUT_TYPES(cls) -> Dict[str, Any]:
        return {
            # required - эти параметры станут виджетами на ноде
            "required": {
                "mode": (
...
- Side-by-Side: Делит вертикально пополам."""
                ...
                "zoom": (
                    "FLOAT", 
                    {
                        ...
                        "tooltip": 
"""Zoom level for both images. 1.0 is 100% (fit to node).
Уровень масштабирования обоих изображений. 1.0 — это 100% (вписать в ноду)."""
                ...
                "pan_x": (
                    "INT", 
                    {
                        ...
                        "tooltip": 
"""Horizontal pan offset in pixels. Useful with zoom > 1.0.
Смещение по горизонтали в пикселях. Полезно при зуме > 1.0."""
                ...
                "pan_y": (
                    "INT", 
                    {
                        "default": 0, "min": -5000, "max": 5000, "step": 1,
                        "tooltip":
"""Vertical pan offset in pixels. Useful with zoom > 1.0.
...
Продвинутый инструмент сравнения изображений. Поддерживает режимы Slider, Difference и Side-by-Side. Включает виджеты Zoom и Pan для изучения деталей. Нажмите 'Space' (Пробел), наведя курсор на ноду, чтобы временно скрыть второе изображение."""
    ...
    def compare_images(
        ...
        mode: str = "Slider",         # <-- Добавлено
        zoom: float = 1.0,            # <-- Добавлено
        pan_x: int = 0,               # <-- Добавлено
        pan_y: int = 0,               # <-- Добавлено
        filename_prefix: str = "AGSoft_Compare_",
        prompt: Optional[Any] = None,
        extra_pnginfo: Optional[Any] = None
    ) -> Tuple[Tuple[Optional[torch.Tensor], Optional[torch.Tensor]], Dict[str, Any]]:
        
        # Словарь для хранения метаданных изображений, которые будут переданы в JS
        ui_data: Dict[str, Any] = {"image_1": [], "image_2": []}
        
        # Обрабатываем первое изображение, если оно подключено
        ...
        # Обрабатываем второе изображение, если оно подключено
        ...
        # Возвращаем результат в специальном формате ComfyUI
        return {
