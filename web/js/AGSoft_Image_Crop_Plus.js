...
function addCustomUI(node) {
    ...
    function loadImageByName(filename) {
        ...
        imgElement.onload = () => {
            ...
            // Увеличенные отступы чтобы изображение не обрезалось
            // Заголовок ноды ~30px + виджеты сверху ~180px + изображение + кнопка сброса ~40px + статусы ~60px + запас ~60px
            const topWidgetsHeight = 220;
            const bottomWidgetsHeight = 120;
            const extraPadding = 80;

            let newHeight = imageDisplayHeight + topWidgetsHeight + bottomWidgetsHeight + extraPadding;
    ...
    imageContainer.addEventListener('dragleave', (e) => {
        e.preventDefault();
        e.stopPropagation();
        imageContainer.style.border = "2px dashed #333";
        ...
        nodeType.prototype.computeSize = function() {
            ...
            if (customWidget && this.domWidgets) {
                ...
                if (widgetElement && widgetElement.element) {
                    const element = widgetElement.element;
                    let elementHeight = element.scrollHeight || element.offsetHeight || element.clientHeight;
                    if (elementHeight > 0) {
                        // Увеличенный запас чтобы ComfyUI не обрезал нижнюю часть
                        size[1] = Math.max(size[1], elementHeight + 100);
                    }
                }
            }
            return size;
