...
function addCustomUI(node) {
    ...
    function updateInfo() {
        if (!imageRect) {
            statusDiv.innerHTML = "⚠️ Нет изображения. Загрузите файл или выберите из списка.";
    ...
    function onMouseMove(e) {
        ...
        if (mode === "Points (4 clicks)" && draggingPoint !== null && dragStart) {
            ...
            if (rectDragType === 'move') {
                ...
                if (isPreset && ratio) {
                    // Для углов и граней сохраняем пропорцию
                    if (rectDragType === 'n' || rectDragType === 's') {
                        // Вертикальная грань - меняем h, w считаем
                        const newW = h * ratio;
                        x = orig.x + (orig.w - newW) / 2;
                        w = newW;
                    } else if (rectDragType === 'e' || rectDragType === 'w') {
                        // Горизонтальная грань - меняем w, h считаем
                        const newH = w / ratio;
                        y = orig.y + (orig.h - newH) / 2;
                        h = newH;
                    } else {
                        // Углы - используем большее изменение
                        ...
                        // Корректируем позицию для углов 'w' и 'n'
    ...
    function onImageClick(e) {
        ...
        const mode = getWidgetValue("crop_mode");
        if (mode !== "Points (4 clicks)") return; // В других режимах клики не ставим
