 ...
 getBounds() {
     ...
     if (app.graph.groups) {
         for (const group of app.graph.groups) {
             if (group._bounding) {
                 const groupLeft = group._bounding[0];
                 const groupTop = group._bounding[1];
                 const groupRight = group._bounding[0] + group._bounding[2];
                 const groupBottom = group._bounding[1] + group._bounding[3];
                 if (groupLeft < minX) minX = groupLeft;
                 if (groupTop < minY) minY = groupTop;
                 if (groupRight > maxX) maxX = groupRight;
                 if (groupBottom > maxY) maxY = groupBottom;
             }
         }
     }
     // Links
