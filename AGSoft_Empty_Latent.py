...
class AGSoft_Empty_Latent_Flux2:
    """
    Создает пустой латентный тензор для моделей FLUX.2.
    
    ОСОБЕННОСТИ FLUX.2:
    1. Тот же размер изображения, что и у SD/SDXL
    2. Размер латента в 4 раза меньше (деление на 16 вместо 8)
    3. Это соответствует уменьшению в 2 раза по каждой оси
    
    Пример:
    - Изображение: 1024x1024 пикселей
    - Латент SD/SDXL: 1024/8 = 128x128
    - Латент FLUX.2: 1024/16 = 64x64 (в 4 раза меньше по площади)
    
    """
    
    # Константы для FLUX.2
    LATENT_FACTOR = 16  # Размер латента = размер изображения / 16
    # Вместо 8 у SD/SDXL
    # Это дает латент в 2 раза меньше по ширине и высоте (4 раза по площади)

    @classmethod
    def INPUT_TYPES(cls):
        """
        Определяет входные параметры ноды FLUX.2.
        
        СОВЕРШЕННО ИДЕНТИЧНЫ параметрам AGSoft_Empty_Latent.
        Меняется ТОЛЬКО размер латента на выходе.
        """
        return {
            "required": {
                "size_mode": (
                ...
                "width": (
                    ...
                    {
                        "default": 1024,
                        "min": 16,  # Минимум 16, т.к. латент = width/16
                        "max": 4096,
                        "step": 8,
                        "step": 8,
                        "tooltip": "Width in pixels (must be divisible by 8).\n"
                ...
                "height": (
                    ...
                    {
                        "default": 1024,
                        "min": 16,  # Минимум 16, т.к. латент = height/16
                        "max": 4096,
                        "step": 8,
                        "tooltip": "Height in pixels (must be divisible by 8).\n"
                                   "Высота в пикселях (должна быть кратна 8)."
    ...
    # Определяем выходы ноды (такие же, как у AGSoft_Empty_Latent)
    RETURN_TYPES = ("LATENT", "INT", "INT", "INT", "INT")
    RETURN_NAMES = ("latent", "width_px", "height_px", "width_latent", "height_latent")
    FUNCTION = "generate"
    CATEGORY = "AGSoft/nodes"
    DESCRIPTION = (
        "Generates an empty latent tensor with specified dimensions for FLUX.2 models.\n"
        "Supports presets, custom size, and megapixel-based resolution.\n\n"
        "Создает пустой латентный тензор с указанными размерами для моделей FLUX.2.\n"
        "Поддерживает пресеты, произвольный ввод и задание разрешения через мегапиксели."
    )

    def generate(self, size_mode, preset, width, height, megapixels, aspect_ratio, divisibility, batch_size):
        """
        Основная функция генерации латентного тензора для FLUX.2.
        
        """
        
        # Преобразуем строковую кратность в число
        divisibility = int(divisibility)
        ...
        if size_mode == "Preset":
            ...
            pass  # width/height уже установлены пользователем
        ...
        # Гарантируем, что размеры кратны 8 (требование SD/FLUX)
        width = max(8, (width // 8) * 8)
        height = max(8, (height // 8) * 8)

        # --- КЛЮЧЕВОЕ ОТЛИЧИЕ ОТ AGSoft_Empty_Latent ---
        # Для FLUX.2: latent space factor = 16 (вместо 8)
        # Это дает латент в 2 раза меньше по ширине и высоте
        # И в 4 раза меньше по площади
        latent_width = width // self.LATENT_FACTOR  # 16 вместо 8!
        latent_height = height // self.LATENT_FACTOR  # 16 вместо 8!
        # --- КОНЕЦ ОТЛИЧИЯ ---

        # Создаем латентный тензор
        # Формат: [batch_size, channels, height, width]
        # FLUX.2 использует 4 канала (так же, как SD/SDXL)
        latent = torch.zeros([batch_size, 4, latent_height, latent_width], device="cpu")

        # Возвращаем результат в том же формате, что и AGSoft_Empty_Latent
        return ({
            "samples": latent
        }, width, height, latent_width, latent_height)
...
NODE_CLASS_MAPPINGS = {
    ...
    "AGSoft_Empty_Latent_Flux2": AGSoft_Empty_Latent_Flux2  # Новая нода для FLUX.2
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "AGSoft_Empty_Latent": "AGSoft Empty Latent",
    "AGSoft_Empty_Latent_QwenImage": "AGSoft Empty Latent QwenImage",
    "AGSoft_Empty_Latent_Flux2": "AGSoft Empty Latent Flux.2"  # Отображаемое имя новой ноды
}
